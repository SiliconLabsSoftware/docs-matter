...
4. **[Optional] Customize Matter App Behavior and Logic**
   ...
   - Put application-specific behavior in `src/CustomerAppTask.cpp` by overriding `*Impl()` hooks, defaults live in `autogen/AppTask.cpp` (reference only, regenerated on upgrade). Add other helper classes under `src/` as needed.
