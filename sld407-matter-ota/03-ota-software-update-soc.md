...
The TLV encryption will be controlled by the `--enc_enable` and `--input_ota_key` parameters passed to the `ota_multi_image_tool.py` script. The AES key will be programmed to the device by the Silicon Labs Matter Provisioning Tool (see [Matter Provisioning](/matter/{build-docspace-version}/matter-provisioning/)). The `ota_key` argument to the provision script should be used to provision the AES key to the device for decrypting OTA payload.
...
## Secure Upgrades on SiWx917 SoC
