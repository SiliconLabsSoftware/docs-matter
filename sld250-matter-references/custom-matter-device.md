...
To customize app behavior through the overridable APIs exposed by each app, see [Matter API Reference](/matter/{build-docspace-version}/matter-api-reference/).
...
Attribute changes route through `CustomerAppTask` and `DMPostAttributeChangeCallbackImpl()`. For more information, see [Matter API Reference](/matter/{build-docspace-version}/matter-api-reference/).
