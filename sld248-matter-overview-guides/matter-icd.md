...
The LIT slow poll is defined via the SL_IDLE_MODE_DURATION_S configuration parameter. The SIT slow poll will automatically default to the maximum SIT slow poll (15 seconds) or to whatever is defined via the CHIP_DEVICE_CONFIG_ICD_SIT_POLLING_INTERVAL configuration parameter. It can be further changed via the [SetSITPollingInterval](https://github.com/SiliconLabs/matter_extension/blob/63c8513144894fe991e2fbbe8998bc6b127adeee/third_party/matter_sdk/src/app/icd/server/ICDConfigurationData.h#L152) API. The SL_ACTIVE_MODE_DURATION_MS and SL_ACTIVE_MODE_THRESHOLD parameters are used to define the fast poll behavior regardless of LIT or SIT mode.
