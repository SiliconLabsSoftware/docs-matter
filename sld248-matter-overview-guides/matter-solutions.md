...
> **Production requirement**: The GBL file generated by Solutions is unsigned by default. For production deployments, the GBL must be signed with your device's ECDSA signing key before being packaged into the OTA file, so the bootloader can verify its authenticity. See [Enabling Secure Upgrades for Production](/matter/{build-docspace-version}/matter-ota/01-ota-bootloader#enabling-secure-upgrades-for-production) for step-by-step instructions.
